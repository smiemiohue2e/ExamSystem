...
<head>
    ...
    <link rel="SHORTCUT ICON"
          href="${pageContext.request.contextPath}/${pageContext.request.contextPath}/static/images/icon.ico">
    <link rel="BOOKMARK"
          href="${pageContext.request.contextPath}/${pageContext.request.contextPath}/static/images/icon.ico">
        ...
        <c:forEach items="${view.singleQuestions}" var="question" varStatus="status">
            <div class="question">
                ...
                <div class="question-answer">
                    答案:
                    <input disabled type="radio" name="single-${status.index}" ${question.answer == '0' ? 'checked' : ''} />A
                    <input disabled type="radio" name="single-${status.index}" ${question.answer == '1' ? 'checked' : ''} />B
                    <input disabled type="radio" name="single-${status.index}" ${question.answer == '2' ? 'checked' : ''} />C
                    <input disabled type="radio" name="single-${status.index}" ${question.answer == '3' ? 'checked' : ''} />D
    ...
    <div id="multi-container">
        ...
        <c:forEach items="${view.multiQuestions}" var="question" varStatus="status">
            <div class="question">
                ...
                <div class="question-answer">
                    答案:
                    <input disabled type="checkbox" value="0" ${fn:contains(question.answer, '0') ? 'checked' : ''} />A
                    <input disabled type="checkbox" value="1" ${fn:contains(question.answer, '1') ? 'checked' : ''} />B
                    <input disabled type="checkbox" value="2" ${fn:contains(question.answer, '2') ? 'checked' : ''} />C
                    <input disabled type="checkbox" value="3" ${fn:contains(question.answer, '3') ? 'checked' : ''} />D
                    <c:if test="${!question.right}">
                        <span class="error">您的答案:${question.wrongAnswerFacade}</span>
                    </c:if>
                </div>
    ...
    <div id="judge-container">
        ...
        <c:forEach items="${view.judgeQuestions}" var="question" varStatus="status">
            <div class="question">
                ...
                <div class="question-answer">
                    答案:
                    <input disabled type="radio" name="judge-${status.index}" ${question.answer == '0' ? 'checked' : ''} />对
                    <input disabled type="radio" name="judge-${status.index}" ${question.answer == '1' ? 'checked' : ''} />错
                    <c:if test="${!question.right}">
                        <span class="error">您的答案:${question.wrongAnswerFacade}</span>
                    </c:if>
                </div>
